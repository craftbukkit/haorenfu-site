  ...
  return (
    <header className='sticky top-0 z-30 border-b border-white/10 bg-bg0/80 backdrop-blur'>
      <div className='mx-auto flex max-w-6xl items-center justify-between px-4 py-3'>
        ...
        <nav className='flex gap-4 text-sm text-mech'>
  ...
  return (
    <section className='relative overflow-hidden px-4 py-20 md:py-28'>
      ...
      <div className='mx-auto max-w-6xl'>
        ...
        <h1 className='mt-4 text-4xl font-bold md:text-6xl'>{serverConfig.serverName}</h1>
        ...
        <div className='mt-8 flex flex-wrap gap-3'>
          <motion.button whileTap={reduce ? {} : { scale: 0.96 }} onClick={async()=>{await navigator.clipboard.writeText(serverConfig.domain);setCopied(true);setTimeout(()=>setCopied(false),1200);}} className='rounded-lg bg-arc px-4 py-2 font-semibold text-bg0'>
            <Copy className='mr-1 inline h-4 w-4'/>复制服务器地址
          </motion.button>
          <a href={`https://qm.qq.com/q/${serverConfig.qqGroup}`} className='rounded-lg border border-white/20 px-4 py-2'>加入 QQ 群 <ExternalLink className='inline h-4 w-4'/></a>
          <Link href='/download' className='rounded-lg border border-white/20 px-4 py-2'>下载完整中文客户端 <Download className='inline h-4 w-4'/></Link>
          <Link href='/guide' className='rounded-lg border border-white/20 px-4 py-2'>查看新手指南</Link>
          <button onClick={()=>setLowFx(!lowFx)} className='rounded-lg border border-white/20 px-4 py-2 text-sm'>{lowFx?'开启动效':'低配模式'}</button>
        </div>
